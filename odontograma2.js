document.addEventListener('DOMContentLoaded', () => {
   ...
   /*  const tamanhoTelaReferencia = 1895
    ...
    const itensProcedimento = [{
        ...
        nome: 'Restauración en buen estado',
        cor: '#0000FF'
    }, {
        ...
        procedimentos.filter(prc => prc.numeroDente === procedimento.numeroDente && prc.faceDente === procedimento.faceDente).forEach(item => {
            const tr = `
                <tr>
                    ...
                    <td>
                        ${item.informacoesAdicionais || 'más infomación'}
                    </td>
                    <td>
    ...
    const resizeCanvas = () => {
        if (window.innerWidth >= 600) {
            ...
            alert("¡PANTALLA MUY PEQUEÑA! ¡Acceda a la carta dental a través de un dispositivo con una pantalla más grande!")
            document.querySelector("#canva-group").style.display = 'none'
        }

        camada1.width = camada2.width = camada3.width = camada4.width = window.innerWidth - 500
        ...
        valoresBase = {
            ...
            x: (camada1.width * 14) / tamanhoTelaReferencia,
            y: (camada1.width * 10) / tamanhoTelaReferencia,
            largura: (camada1.width * 10) / tamanhoTelaReferencia,
            altura: (camada1.width * 10) / tamanhoTelaReferencia
        }

        base_image = new Image();
        /* base_image.src = 'images/dentes/normas.png'; */
        base_image.onload = function() {
            contexto1.drawImage(base_image, valoresBase.x, valoresBase.y, valoresBase.largura, valoresBase.altura);
        }

        posicoesPadrao.margemXEntreDentes = (camada1.width * 15) / tamanhoTelaReferencia
        posicoesPadrao.margemYEntreDentes = (camada1.width * 150) / tamanhoTelaReferencia
        posicoesPadrao.posicaoYInicialDente = (camada1.width * 130) / tamanhoTelaReferencia
    ...
    const resizeCanvasPincel = () => {
        ...
        camadaPincel.width = window.innerWidth - 500
